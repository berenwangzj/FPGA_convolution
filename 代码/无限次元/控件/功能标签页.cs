...
public partial class 功能标签页 : UserControl {
    ...
    private void 读参数() {
        try {
            (功能参数.宽, 功能参数.高) = (Convert.ToInt32(图宽.Text), Convert.ToInt32(图高.Text));
            功能参数.颜色 = Color.FromArgb(Convert.ToByte(R值.Text), Convert.ToByte(G值.Text), Convert.ToByte(B值.Text));
            (功能参数.红比例, 功能参数.绿比例, 功能参数.蓝比例) = (Convert.ToSingle(R比例.Text), Convert.ToSingle(G比例.Text), Convert.ToSingle(B比例.Text));
            (功能参数.W, 功能参数.b) = (Convert.ToSingle(W值.Text), Convert.ToSingle(Bias值.Text));
            功能参数.HSL上限 = Color.FromArgb(Convert.ToByte(H上.Text), Convert.ToByte(S上.Text), Convert.ToByte(L上.Text));
            功能参数.HSL下限 = Color.FromArgb(Convert.ToByte(H下.Text), Convert.ToByte(S下.Text), Convert.ToByte(L下.Text));
            功能参数.显示连通域标记过程 = 是否显示过程.Checked;
            功能参数.算子直径 = Convert.ToInt32(算子直径.Text);
            功能参数.胀蚀阈值 = Convert.ToInt32(胀蚀阈值.Text);
            if (功能参数.胀蚀阈值 == 0) { 功能参数.胀蚀阈值 = 1; 胀蚀阈值.Text = "1"; }
            if (功能参数.胀蚀阈值 > 功能参数.算子直径 * 功能参数.算子直径) { 功能参数.胀蚀阈值 = 功能参数.算子直径 * 功能参数.算子直径; 胀蚀阈值.Text = 功能参数.胀蚀阈值.ToString(); }
        }
        catch (Exception exp) {
    ...
    private void 位宽选择_Click(object sender, EventArgs e) {
        ...
        if (位宽选择.Text == "32位")
            功能参数.像素格式 = PixelFormat.Format32bppArgb;
        else
            功能参数.像素格式 = PixelFormat.Format24bppRgb;
    }//选择24位还是32位图
    ...
    private void 胀蚀阈值_TextChanged(object sender, EventArgs e) {
        胀蚀阈值.Text = 限制字符为正整数(胀蚀阈值.Text, 功能参数.算子直径 * 功能参数.算子直径); //阈值的最大值是直径的平方
    }
    ...
    private void H使能_CheckedChanged(object sender, EventArgs e) {
        功能参数.H使能 = H使能.Checked;
        功能参数.S使能 = S使能.Checked;
        功能参数.L使能 = L使能.Checked;
    }
