...
public class Mnist查看标签页 : TabPage, I标签页 {
    public static string Mnist路径 = @"C:\PyMnist\data\MNIST\raw\"; //改为Mnist数据所在文件夹
    public static string 训练图片名 = @"train-images-idx3-ubyte";
        ...
        unsafe {
            ...
            for (int y = 0; y < 图数据.Height; y++)  //这里必须用"位图数据.Height",不小心写成"位图.Height"就会导致程序莫名变慢很多.
                for (int x = 0; x < 图数据.Width; x++, 图指针 += 4) {
                    ...
                    if (x == 图数据.Width - 1 || y == 图数据.Height - 1 || x % 图尺寸 == 0 || y % 图尺寸 == 0) {
