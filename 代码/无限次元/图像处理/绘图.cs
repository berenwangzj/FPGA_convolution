...
public class 绘图 {
    ...
    static void G画点(Graphics g, Point 点, Color 颜色) {
        G画点(g, 点.X, 点.Y, 颜色);
    }

    static public void G画点(Graphics g, float x, float y, Color 颜色) {
        g.DrawLine(new Pen(new SolidBrush(颜色)), (int)(x + 0.5), (int)(y + 0.5), (int)(x + 0.5), (float)((int)(y + 0.5) - 0.2));
    }
    static public void 画整型数据(int[] 数据, Graphics g, Rectangle 绘图边框, Color 线条颜色, float 线粗 = 1) {
        Pen 粗笔 = new Pen(线条颜色, 线粗);
        float X单位 = (float)绘图边框.Width / 数据.Length;
        float Y单位 = (float)绘图边框.Height / 数据.Max();
        for (int i = 0; i < 数据.Length - 1; i++)
            g.DrawLine(粗笔, i * X单位 + 绘图边框.X, 绘图边框.Height + 绘图边框.Y - 数据[i] * Y单位, (i + 1) * X单位 + 绘图边框.X, 绘图边框.Height + 绘图边框.Y - 数据[i + 1] * Y单位);
    }
    ...
    static public void 画柱状图(Graphics g, double[] 数据, double 数据最大值, double 数据最小值, Rectangle 边框, String 图名, Color 柱色) {
        ...
        for (int i = 0; i < 数据.Length; i++) {
            ...
            if (i % 4 == 0 || i < 10)
                g.DrawString(i.ToString(), TimesNR字体(8), new SolidBrush(Color.Purple), new Point(边框.X + i * 宽 - 3, 边框.Y + 边框.Height));
        }
        g.DrawString(图名, TimesNR字体(8), new SolidBrush(Color.Purple), new Point(边框.X - 3, 边框.Y - 15));
    }
