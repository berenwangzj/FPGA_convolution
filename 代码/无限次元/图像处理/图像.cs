...
public partial class 图像 {
    ...
    static public void 点运算(ref byte Red, ref byte Green, ref byte Blue) {
        (int R, int G, int B) = ((int)(Red * 功能参数.W + 功能参数.b + 0.5), (int)(Green * 功能参数.W + 功能参数.b + 0.5), (int)(Blue * 功能参数.W + 功能参数.b + 0.5));
    ...
    static public void 转灰度(ref byte Red, ref byte Green, ref byte Blue) {
        (int R, int G,int B) = ((int)(Red * 功能参数.红比例 + 0.5), (int)(Green * 功能参数.绿比例 + 0.5), (int)(Blue * 功能参数.蓝比例 + 0.5));
        int Gray = R + G + B;
        ...
        unsafe {
            ...
            for (int y = 1; y < 位图数据.Height - 1; y++)
                for (int x = 1; x < 位图数据.Width - 1; x++) {
                    ...
                    int Gx = 右上 - 左上 + 2 * (右 - 左) + (右下 - 左下);
                    int Gy = 右下 - 右上 + 2 * (下 - 上) + (左下 - 左上);
        ...
        unsafe {
            ...
            for (int y = 0; y < 位图数据.Height; y++, 位针 += 位图数据.Stride - 位图数据.Width * 字节数)
                for (int x = 0; x < 位图数据.Width; x++, 位针 += 字节数) {
                    int H = 0, S = 0, L = 0;
                    RGB转HSL(位针[红], 位针[绿], 位针[蓝], ref H, ref S, ref L);
                    if ((H <= 功能参数.HSL上限.R && H >= 功能参数.HSL下限.R || !功能参数.H使能) &&
                        (S <= 功能参数.HSL上限.G && S >= 功能参数.HSL下限.G || !功能参数.S使能) &&
                        (L <= 功能参数.HSL上限.B && L >= 功能参数.HSL下限.B || !功能参数.L使能)) {
                        if (二值化)
                            位针[红] = 位针[绿] = 位针[蓝] = 0; //背景,黑
                        else
                            (位针[红], 位针[绿], 位针[蓝]) = (功能参数.颜色.R, 功能参数.颜色.G, 功能参数.颜色.B);
                    }
                    else {
                        if (二值化)
    ...
    static public void 读Yolo标签(Bitmap 位图, string 标签文件) {
        ...
        while ((一行 = 读标签.ReadLine()) != null) {
            ...
            g.DrawString(标签, 绘图.TimesNR字体(14), new SolidBrush(Color.Red), new Point(边框.X, 边框.Y - 20));
        }
        g.Dispose();
    }
