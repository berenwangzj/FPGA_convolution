...
public partial class 图像 {
    ...
    static public void 递归连通(Bitmap 位图, PictureBox 图片框) {
        if (!功能参数.显示连通域标记过程) 图片框 = null;
        int[,] 二值数组 = 连通域扫描(位图, 图片框);
        if (二值数组 == null) { MessageBox.Show("这不是二值图像!"); return; } 
        连通域信息反标位图(位图, 二值数组);
    }

    static int[,] 连通域扫描(Bitmap 位图, PictureBox 图片框) {
        int[,] 二值数组 = new int[位图.Height, 位图.Width];
        if (!位图转二值数组(位图, 二值数组, 1, 0)) return null;
        int 标号 = 2;                                           //0:背景, 1: 前景, 所以连通域标号从2开始
        for (int y = 1; y < 二值数组.GetLength(0) - 1; y++)     //GetLength:[0,1]
            for (int x = 1; x < 二值数组.GetLength(1) - 1; x++)
                if (二值数组[y, x] == 1) {
                    int 计数 = 0, 邻域索引 = 0;
                    递归标邻域(二值数组, x, y, 标号++, ref 计数, ref 邻域索引, 位图, 图片框);
                }
        return 二值数组;
    }
    static void 递归标邻域(int[,] 二值数组, int x, int y,  int 标号, ref int 计数, ref int 邻域索引, Bitmap 位图 = null, PictureBox 图片框 = null) {
        二值数组[y, x] = 标号;
        
        if(图片框 != null) {
            位图.SetPixel(x , y, 色谱颜色24[(标号 - 2) % 24]);
            图片框.Invalidate(new Rectangle(x - 2, y - 2, 3, 3));
            图片框.Update();
        }//显示过程
        ...
        foreach (Point 邻域点 in 顺八邻域[邻域索引]) {
            if (二值数组[y + 邻域点.Y, x + 邻域点.X] == 1)
                递归标邻域(二值数组, x + 邻域点.X, y + 邻域点.Y, 标号, ref 计数, ref 邻域索引, 位图, 图片框);
            邻域索引 = (邻域索引 + 1) % (顺八邻域.Length); //延迟爆栈措施
        }
        计数 = 0; //延迟爆栈措施
    }//递归法的坏处是连通域点数太多就会栈溢出,所以加了点延迟爆栈措施,连通域太大终究还是会爆.

    private static void 连通域信息反标位图(Bitmap 位图, int[,] 二值数组) {
        List<连通域信息> 连通域表 = new();
        byte 字节数 = 3;
        if (位图.PixelFormat == PixelFormat.Format32bppArgb) 字节数 = 4;
        BitmapData 位图数据 = 位图.LockBits(new Rectangle(0, 0, 位图.Width, 位图.Height), ImageLockMode.ReadWrite, 位图.PixelFormat);
        ...
        for (int i = 0; i < 连通域表.Count; i++) {
            ...
            //画边框
            g.DrawRectangle(new Pen(Color.LightGray), new Rectangle(连通域表[i].XMin - 1, 连通域表[i].YMin - 1, 连通域表[i].XMax - 连通域表[i].XMin + 2, 连通域表[i].YMax - 连通域表[i].YMin + 2));
        }
        g.Dispose();
