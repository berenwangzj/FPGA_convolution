...
public partial class 无限主窗体 : Form {
    ...
    private void 功能标签页1_图像处理按钮点击(object sender, EventArgs e) {
        ...
        if (功能 == "变透明") {
            ...
            计时开始((Button)sender);
            图像.连通域识别(当前图片页.图数据.位图, 刷新图片, 功能);
            计时结束((Button)sender);
        }
        else if(功能 == "连通标结果") {
            if(!File.Exists(@"D:\FPGA\ImageProcess\CCList.txt")) { MessageBox.Show("请在代码里修改输出文件路径！"); return; }
            图像.读连通域FPGA仿真结果(当前图片页.图数据.位图, @"D:\FPGA\ImageProcess\CCList.txt");
        }
        else
            try {
